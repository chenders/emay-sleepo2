...
import { parseReading, checksum, command, HELLO, HEARTBEAT, START_REALTIME, DEVICE_STATE, STOP_REALTIME } from "../src/protocol.js";
...
describe("checksum", () => {
  ...
  it("startRealtime", () => assert.equal(checksum(Buffer.from([0x9B, 0x01])), 0x1C));
  it("stopRealtime", () => assert.equal(checksum(Buffer.from([0x9B, 0x7F])), 0x1A));
});
...
describe("parseReading", () => {
  ...
  it("wrong length", () => assert.equal(parseReading(Buffer.from([0xEB, 0x01])), null));
  it("bad checksum", () => assert.equal(parseReading(Buffer.from([0xEB, 0x01, 0x05, 62, 98, 0x7F, 0x00, 0xFF])), null));
  ...
  it("pulse < 30 rejected", () => assert.equal(parseReading(frame(29, 98)), null));
  it("pulse > 220 rejected", () => assert.equal(parseReading(frame(221, 98)), null));
  ...
  it("SpO2 > 100 rejected", () => assert.equal(parseReading(frame(62, 101)), null));
});
